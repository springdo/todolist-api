pipeline {
    ...
    environment {
        // TODO - remove this hard ref
        PROJECT_NAMESPACE = "donal-dev"
        ...
        GIT_URL = "gitlab-donal-ci-cd.apps.s8.core.rht-labs.com/donal/"
    }
    ...
    stages {
        stage("prepare environment for master deploy") {
            ...
            steps {
                script {
                  ...
                  env.PROJECT_NAMESPACE = "donal-test"
        ...
        stage("node-build") {
            ...
            steps {
                git branch: 'develop',
                    ...
                    url: 'https://gitlab-donal-ci-cd.apps.s8.core.rht-labs.com/donal/todolist-api.git'
                sh 'printenv'
            ...
            post {
                ...
                success {
                    ...
                    sh  '''
                            ...
                            # git push https://gitlab.apps.xpertex.rht-labs.com/donal/${APP_NAME}.git --tags
                            git push https://${GIT_CREDENTIALS_USR}:${GIT_CREDENTIALS_PSW}@${GIT_URL}${APP_NAME}.git --tags
                        '''
                }
                failure {
                    echo "FAILURE"
        ...
        stage("node-bake") {
            ...
            steps {
                ...
                sh  '''
                        oc project donal-ci-cd # probs not needed
        ...
        stage("node-deploy") {
            ...
            steps {
                ...
                sh '''
                    oc set env dc ${APP_NAME} NODE_ENV=dev
